...
class SignupBloc extends Bloc<SignupEvent, SignupState> {
  ...
  SignupBloc({required LoginRepository loginRepository}) :
        _loginRepository=loginRepository,super(SignupInitial()) {
  ...
  void _onValidation(SignupValidation event, Emitter<SignupState> emit) async {
    ...
    favHastag.add("0");
    if (event.fullName.isEmpty && event.password.isEmpty && event.mobile.isEmpty && event.dob.isEmpty) {
      emit( const SignupError(message: "Field required", isPass: true, isDob: true, isName: true, isMobile: true));
    } else if (event.fullName.isEmpty) {
      emit( const SignupError(message: "Field required", isPass: false, isDob: false, isName: true, isMobile: false));
    } else if (event.fullName.isEmpty  && event.dob.isEmpty) {
      emit( const SignupError(message: "Field required", isPass: false, isDob: true, isName: true, isMobile: false));
    } else if (event.fullName.isEmpty && event.password.isEmpty) {
      emit( const SignupError(message: "Field required", isPass: true, isDob: false, isName: true, isMobile: false));
    } else if (event.mobile.isEmpty && event.password.isEmpty && event.dob.isEmpty) {
      emit( const SignupError(message: "Field required", isPass: true, isDob: true, isName: false, isMobile: true));
    } else if (event.fullName.isEmpty && event.password.isEmpty && event.dob.isEmpty) {
      emit( const SignupError(message: "Field required", isPass: true, isDob: true, isName: true, isMobile: false));
    } else if (event.mobile.isEmpty ) {
      emit( const SignupError(message: "Field required", isPass: false, isDob: false, isName: false, isMobile: true));
    } else if (event.mobile.length < 10 ) {
      emit( const SignupError(message: "Mobile must be 10 digit", isPass: false, isDob: false, isName: false, isMobile: true));
    } else if (event.dob.isEmpty) {
      emit( const SignupError(message: "Field required", isPass: false, isDob: true, isName: false, isMobile: false));
    } else if (event.dob.isEmpty && event.password.isEmpty) {
      emit( const SignupError(message: "Field required", isPass: true, isDob: true, isName: false, isMobile: false));
    } else if (event.password.isEmpty) {
      emit( const SignupError(message: "Field required", isPass: true, isDob: false, isName: false, isMobile: false));
