...
class LoginBloc extends Bloc<LoginEvent, LoginState> {
  ...
  void _onTextChange(TextChangeEvent event, Emitter<LoginState> emit) async {
    ...
    if (event.loginType == 'normal') {
      if (event.email.isEmpty && event.password.isEmpty) {
        ...
        emit(LoginValidated());
        var result =
            await _loginRepository.loginUser(event.email, event.password);
        if (result['status']) {
          try {
            ...
            await pref.setString(
                '${user.id}currentToken', result['data']['token']);
      ...
      try {
        ...
        if (account != null) {
          var result = await _loginRepository.socialLoginRegister(
              account.id, 'google', account.email, account.displayName ?? '');
          if (result['status']) {
            var pref = await SharedPreferences.getInstance();
            try {
              UserModel user = UserModel.fromJson(result['data']['user']);
      ...
      if (result.status.toString() == 'LoginStatus.success') {
        ...
        emit(const LoginStatus(
            message: 'Error Try after some time', status: false));
      }
    }
  }

  void _onPhoneValidation(
      PhoneValidation event, Emitter<LoginState> emit) async {
  ...
  void _onPasswordValidation(
      PassValidation event, Emitter<LoginState> emit) async {
    if (event.pass.isEmpty && event.confirm.isEmpty) {
      emit(const OnPassValidation(
          message: "Field required", isPass: true, isConfirm: true));
    } else if (event.pass.length < 4) {
      emit(const OnPassValidation(
          message: "New M-Pin must be 4 digit",
          isPass: true,
          isConfirm: false));
    } else if (event.confirm.isEmpty) {
      emit(const OnPassValidation(
          message: "Field required", isPass: false, isConfirm: true));
    } else if (event.confirm.length < 4) {
      emit(const OnPassValidation(
          message: "Confirm M-Pin must be 4 digit",
          isPass: false,
          isConfirm: true));
    } else if (event.confirm != event.pass) {
      emit(const OnPassValidation(
          message: "M-Pin did`t match", isPass: false, isConfirm: true));
    } else {
      emit(LoginValidated());
