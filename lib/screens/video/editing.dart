...
class _EditingState extends State<Editing> {
  ...
  void initState() {
    ...
    videoPlayerController = VideoPlayerController.file(
      ...
      ..initialize().then((value) {
        ...
        sfRangeValues =
            SfRangeValues(0, videoPlayerController.value.duration.inSeconds);
        rangeController = RangeController(
            start: 0, end: videoPlayerController.value.duration.inSeconds);
        isVidInit = true;
        setState(() {});
      });
    try {
                      ...
                      child: isVidInit
                          ? SfRangeSelector(
                              ...
                              child: Container(
                                decoration: BoxDecoration(
                                    border: Border.all(
                                        color: ColorManager.cyan, width: 2.5)),
                                height: 50,
                              ),
                            )
                          : const SizedBox()),
                        ...
                        child: Row(
                          children: [
                            Radio(
                              ...
                              onChanged: (int? val) =>
                                  setState(() => radioGroupValue = val ?? 0),
  ...
  createThumbs() async {
    ...
    String outputPath =
        "$saveCacheDirectory${dateTime.day}${dateTime.month}${dateTime.year}${dateTime.hour}${dateTime.minute}${dateTime.second}/";
    directory = Directory(outputPath);
    if (!directory.existsSync()) {
      directory.createSync();
    }
    FFmpegKit.execute(
            "-i ${widget.data.filePath} -r $frameRate -f image2 ${outputPath}image-%3d.png")
        .then((session) async {
  ...
  continuePressed() async {
    ...
    PostData newPostData = PostData(
      ...
      isDuet: false,
      trimStart: rangeController.end - rangeController.start < 15
          ? 0
          : rangeController.end - rangeController.start > 60
              ? 0
              : rangeController.start.toInt(),
      trimEnd: rangeController.end - rangeController.start < 15
          ? 15
          : rangeController.end - rangeController.start > 60
              ? 60
              : rangeController.end.toInt(),
      isDefaultSound: radioGroupValue == 0 ? true : false,
      isUploadedFromGallery: widget.data.isUploadedFromGallery,
    );
    await Get.to(Preview(data: widget.data));
    videoPlayerController = VideoPlayerController.file(
        File(widget.data.filePath))
      ..initialize().then((value) {
        videoPlayerController.play();
