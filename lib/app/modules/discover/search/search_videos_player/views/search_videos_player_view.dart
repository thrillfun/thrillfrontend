...
import '../../../../profile/liked_video_player/controllers/liked_video_player_controller.dart';
...
class SearchVideosPlayerView extends GetView<SearchVideosPlayerController> {
  SearchVideosPlayerView({Key? key}) : super(key: key);
      ...
      body: controller.obx(
          (state) => LoopPageView.builder(
              ...
              itemBuilder: (context, index) {
                ...
                return SearchVideos(
                  videoUrl: state[0]!.videos![index].video.toString(),
                  pageController: pageViewController!,
                  nextPage: index + 1,
                  videoId: state[0]!.videos![index].id!,
                  gifImage: state[0]!.videos![index].gifImage,
                  soundName: state[0]!.videos![index].soundName,
                  UserId: state[0]!.videos![index].user!.id,
                  userName: state[0]!.videos![index].user!.username!.obs,
                  description: state[0]!.videos![index].description!.obs,
                  hashtagsList: state[0].videos![index].hashtags ??
                      [], //json problem name not coming up
                  soundOwner: state[0]!.videos![index].soundOwner,
                  sound: state[0]!.videos![index].sound,
                  videoLikeStatus:
                      state[0]!.videos![index].videoLikeStatus.toString(),
                  isCommentAllowed:
                      state[0]!.videos![index].isCommentable == "Yes"
                          ? true.obs
                          : false.obs,
                  like: state[0]!.videos![index].likes!.obs,
                  isfollow: state[0]
                      .videos![index]
                      .user!
...
class _SearchVideosState extends State<SearchVideos>
  ...
  var relatedVideosController = Get.find<SearchVideosPlayerController>();
                      ...
                      children: [
                        ...
                        Visibility(
                          visible: widget.hashtagsList != null &&
                              widget.hashtagsList!.isNotEmpty,
                          child: Container(
                            height: 35,
...
class SearchCommentsView extends GetView<CommentsController> {
  SearchCommentsView(
      {Key? key,
      this.videoId,
      this.userId,
