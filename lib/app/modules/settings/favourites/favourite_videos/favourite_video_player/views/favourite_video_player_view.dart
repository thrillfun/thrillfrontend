...
class FavouriteVideoPlayerView extends GetView<FavouriteVideoPlayerController> {
  const FavouriteVideoPlayerView({Key? key}) : super(key: key);
  @override
  Widget build(BuildContext context) {
    var pageViewController = PageController();
    var playerController = BetterPlayerListVideoPlayerController();
    return PageView.builder(
        itemCount: (Get.arguments["favourite_videos"] as List<FavouriteVideos>).length,
        scrollDirection: Axis.vertical,
        controller: pageViewController,
        itemBuilder: (context, index) => BetterPlayerListVideoPlayer(
          BetterPlayerDataSource.network(RestUrl.videoUrl +
              (Get.arguments["favourite_videos"] as List<FavouriteVideos>)[index].video!),
