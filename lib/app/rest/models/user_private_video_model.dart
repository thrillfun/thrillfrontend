class UserPrivateVideosModel {
  bool? status;
  String? message;
  List<PrivateVideos>? data;
  bool? error;

  UserPrivateVideosModel({this.status, this.message, this.data, this.error});

  UserPrivateVideosModel.fromJson(Map<String, dynamic> json) {
    status = json['status'];
    message = json['message'];
    if (json['data'] != null) {
      data = <PrivateVideos>[];
      json['data'].forEach((v) {
        data!.add(new PrivateVideos.fromJson(v));
      });
    }
    error = json['error'];
  }
