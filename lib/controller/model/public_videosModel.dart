class PublicVideosModel {
  bool? status;
  String? message;
  List<PublicVideos>? data;
  bool? error;

  PublicVideosModel({this.status, this.message, this.data, this.error});

  PublicVideosModel.fromJson(Map<String, dynamic> json) {
    status = json['status'];
    message = json['message'];
    if (json['data'] != null) {
      data = <PublicVideos>[];
      json['data'].forEach((v) {
        data!.add(new PublicVideos.fromJson(v));
      });
    }
    error = json['error'];
  }
...
class PublicUser {
  ...
  PublicUser.fromJson(Map<String, dynamic> json) {
    ...
    likes = json['likes']??"";
    levels =
        json['levels'] != null ? new PublicLevels.fromJson(json['levels']) : null;
